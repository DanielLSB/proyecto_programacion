...
public:
	...
	void deletV(int id_aux) {
		...
		if (cn.getConectar()) {
			...
			ss = "DELETE FROM ventas WHERE id_venta= " + id1 + " ";
			string query = ss.c_str();
			const char* q = query.c_str();
			q_estado = mysql_query(cn.getConectar(), q);
			if (!q_estado)
			{
				cout << "REGISTRO DE VENTA ELIMINADO . . ." << endl;
	...
	void leerTVent() {
		...
		gotoxy(10, 1); cout <<"NO_FAC"<< endl;
		gotoxy(17, 1); cout <<"SR"<< endl;
		gotoxy(22, 1); cout <<"FECH_fA"<< endl;
		gotoxy(35, 1); cout <<"CLIENTE" << endl;
		gotoxy(60, 1); cout <<"NIT"<<endl;
		gotoxy(72, 1); cout <<"EMPLEADO"<< endl;
		gotoxy(95, 1); cout <<"FECH_INGR"<< endl;


		cn.abrir_conexion();
		if (cn.getConectar()) {
			string consulta = "select c.id_venta,c.no_factura,c.serie,c.fecha_factura,p.nombres,m.nit,f.nombres,c.fecha_ingreso from ventas as c inner join clientes as p on c.id_cliente = p.id_cliente inner join clientes as m on c.id_cliente = m.id_cliente inner join empleados as f on c.id_empleado = f.id_empleado ORDER BY c.id_venta ASC; ";
			const char* c = consulta.c_str();
			...
			if (!q_estado) {
				...
				while (fila = mysql_fetch_row(resultado)) {
					...
					gotoxy(10, cl); cout << fila[1] << endl;
					gotoxy(17, cl); cout << fila[2] << endl;
					gotoxy(22, cl); cout << fila[3] << endl;
					gotoxy(35, cl); cout << fila[4] << endl;
					gotoxy(60, cl); cout << fila[5] << endl;
					gotoxy(72, cl); cout << fila[6] << endl;
