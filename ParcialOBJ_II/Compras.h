...
public:
	...
	void leerComp() {
		...
		gotoxy(8, 1); cout <<"NO_ORD"<< endl;
		gotoxy(14, 1); cout <<"PROVEEDOR"<< endl;
		gotoxy(35, 1); cout <<"DIRECCION"<< endl;
		gotoxy(65, 1); cout <<"FECHA_OR"<< endl;
		gotoxy(85, 1); cout <<"FECHA_INGRE"<< endl;


		cn.abrir_conexion();
		if (cn.getConectar()) {
			string consulta = "select c.id_compra,c.no_orden_compra,p.proveedor,p.direccion,c.fecha_orden,c.fecha_ingreso from compras as c inner join proveedores as p on c.id_proveedor = p.id_proveedor	ORDER BY c.id_compra ASC; ";
			const char* c = consulta.c_str();
			...
			if (!q_estado) {
				...
				while (fila = mysql_fetch_row(resultado)) {
					...
					gotoxy(14, cl); cout << fila[2] << endl;
					gotoxy(35, cl); cout << fila[3] << endl;
					gotoxy(65, cl); cout << fila[4] << endl;
					gotoxy(85, cl); cout << fila[5] << endl;
				}
	...
	void deletVD(int id_aux) {
		...
		if (cn.getConectar()) {
			...
			ss = "DELETE FROM compras WHERE id_compra= " + id1 + " ";
